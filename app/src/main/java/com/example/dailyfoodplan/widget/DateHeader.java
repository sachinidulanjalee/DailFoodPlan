...
public class DateHeader extends LinearLayout {
    ...
    public void setServings(final int servingsOnDate) {
        ...
        if (inDailyDozenMode()) {
            tvStar.setVisibility(servingsOnDate == com.example.dailyfoodplan.Common.MAX_SERVINGS ? VISIBLE : GONE);
        }
    ...
    private boolean inDailyDozenMode() {
        return max == com.example.dailyfoodplan.Common.MAX_SERVINGS;
    }
    ...
    public void onSubHeaderClicked() {
        ...
        if (inDailyDozenMode()) {
            if (!DDServings.isEmpty()) {
                com.example.dailyfoodplan.Common.openServingsHistory(context);
            } else {
                com.example.dailyfoodplan.Common.showToast(context, R.string.no_servings_recorded);
            }
        } else {
            if (!TweakServings.isEmpty()) {
                com.example.dailyfoodplan.Common.openTweakServingsHistory(context);
            } else {
                com.example.dailyfoodplan.Common.showToast(context, R.string.no_servings_recorded);
            }
        }
